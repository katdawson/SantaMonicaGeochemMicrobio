...
library(pacman)
pacman::p_load(reshape2, ggplot2, vegan, gtools, stats, superheat, RColorBrewer, Heatplus, gplots, corrplot, xlsx, Hmisc, igraph, network)
```
...
# Pick a threshold for what pearson correlation level will be plotted, here it is Pearson>0.5
...
E(net)$width=5*as.numeric(edges_highpos1[,3])
plot.igraph(net, layout=layout.fruchterman.reingold, vertex.color="gray80", edge.color="darkorange")
```
---